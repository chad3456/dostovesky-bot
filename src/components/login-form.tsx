    ...
    <div className="mt-8 space-y-4">
      ...
      {!googleEnabled && !testEnabled && (
        <p className="rounded-lg bg-amber-50 p-3 text-center text-sm text-amber-700">
          No sign-in method is configured. Set Google OAuth credentials in your
          environment.
        </p>
      )}
      ...
      {testEnabled && (
        <>
          ...
          <form onSubmit={handleTest} className="space-y-3" aria-label="Test login">
            ...
            <input
              ...
              className="w-full rounded-xl border border-slate-300 px-4 py-2.5 text-slate-800 focus:border-brand-500"
              aria-label="Email"
            />
            <input
              type="text"
            ...
            >
              {loading === "test" ? "Signing in…" : "Continue"}
            </button>
          </form>
        </>
      )}
