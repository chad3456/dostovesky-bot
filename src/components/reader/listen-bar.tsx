...
export function ListenBar({ tts }: { tts: TtsController }) {
  return (
    <div className="fixed inset-x-0 bottom-0 z-30 flex justify-center px-4 pb-4">
      <div className="flex w-full max-w-md flex-wrap items-center gap-x-3 gap-y-2 rounded-2xl bg-slate-900/95 px-4 py-3 text-white shadow-2xl">
        ...
        {tts.voices.length > 0 && (
          <label className="flex min-w-0 flex-1 items-center gap-1.5 text-xs">
            ...
            <select
              ...
              className="min-w-0 flex-1 truncate rounded-lg bg-white/10 px-2 py-1 text-xs text-white outline-none"
            >
              {tts.voices.map((v) => (
